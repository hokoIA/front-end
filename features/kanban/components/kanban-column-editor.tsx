...
export function KanbanColumnEditor({
  ...
  const formKey = open ? `${mode}-${column?.id ?? "new"}` : "closed";

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {open ? (
        <KanbanColumnEditorForm
          key={formKey}
          mode={mode}
          column={column}
...
function KanbanColumnEditorForm({
  mode,
  column,
  onOpenChange,
  onSubmit,
  ...
  const [name, setName] = useState(
    mode === "edit" && column ? column.name : "",
  );
  const [color, setColor] = useState(
    mode === "edit" && column ? column.color || DEFAULT_COLOR : DEFAULT_COLOR,
  );
